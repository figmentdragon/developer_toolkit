/* global MYTHEMEIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'MYTHEME-supports-dark-theme' ) ) {
	MYTHEMEDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function MYTHEMEDarkModeEditorInit( attempt ) {
	...
	if ( MYTHEMEIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						MYTHEMEDarkModeEditorInit( attempt + 1 );
					},
