...
class pencilTwitterOAuth
{
	...
	/** @var writing_Twitter_OAuthConsumer */
	private $consumer;
	 ...
	 * @throws writing_TwitterException when CURL extension is not loaded
	 */
	public function __construct($consumerKey, $consumerSecret, $accessToken = null, $accessTokenSecret = null)
	{
		if (!extension_loaded('curl')) {
			throw new writing_TwitterException('PHP extension CURL is not loaded.');
		}

		$this->consumer = new writing_Twitter_OAuthConsumer($consumerKey, $consumerSecret);
		$this->token = new writing_Twitter_OAuthConsumer($accessToken, $accessTokenSecret);
	}
	 ...
	 * @return bool
	 * @throws writing_TwitterException
	 */
	...
	public function request($resource, $method, array $data = null, array $files = null)
	{
		...
		foreach ((array) $data as $key => $val) {
			if ($val === null) {
				...
				throw new writing_TwitterException('Due to limitation of cURL it is not possible to send message starting with @ and upload file at the same time in PHP < 5.5');
			}
		}

		foreach ((array) $files as $key => $file) {
			if (!is_file($file)) {
				throw new writing_TwitterException("Cannot read the file $file. Check if file exists on disk and check its permissions.");
			}
			$data[$key] = $hasCURLFile ? new CURLFile($file) : '@' . $file;
		}

		$request = writing_Twitter_OAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $resource, $files ? [] : $data);
		$request->sign_request(new writing_Twitter_OAuthSignatureMethod_HMAC_SHA1, $this->consumer, $this->token);
		...
		if (curl_errno($curl)) {
			throw new writing_TwitterException('Server error: ' . curl_error($curl));
		}
		...
		if ($payload === false) {
			throw new writing_TwitterException('Invalid server response');
		}

		$code = curl_getinfo($curl, CURLINFO_HTTP_CODE);
		if ($code >= 400) {
			throw new writing_TwitterException(isset($payload->errors[0]->message)
 ...
 */
class writing_TwitterException extends Exception
{
}