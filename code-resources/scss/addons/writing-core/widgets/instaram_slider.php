...
class Asalah_InstagramSlider extends WP_Widget {
		...
		public function form( $instance ) {
			...
			<div class="jr-container">
				...
				<p>
				<p>
					<label for="<?php echo $this->get_field_id( 'access_token' ); ?>"><strong><?php _e('Access Token:', 'jrinstaslider'); ?></strong> due to Instagram API updates, you will need to get instagram token access for the plugin to work, please <a href="https://www.youtube.com/watch?v=uj9WpuFIZb8">this video</a> to get it.</label>
		...
		private function display_images( $args ) {
			...
			$access_token         = isset( $args['access_token'] ) && !empty( $args['access_token'] ) ? $args['access_token'] : false;
			...
			if ( 'image_link' == $images_link ) {
				...
				$link_attr = 'https://www.instagram.com/{{username}}';
			} elseif ( 'image_url' == $images_link ) {
				$link_attr = "{{link}}";
			} elseif ( 'custom_url' == $images_link ) {
				$link_attr = $custom_url;
			} else {
				$link_attr = '';
			}

			if ( $images_link ) {
				$image_list_start = '<a href="' . $link_attr . '" target="_blank"';
				...
				$image_list_start .= ' title="{{caption}}">';
				$image_list_end = '</a>';
			} else {
				$image_list_start = '';
				$image_list_end = '';
			...
			if ( $template == 'slider' || $template == 'slider-overlay' ) {
					...
					if ( $template == 'slider-overlay' ) {
						$caption_list .= "<div class=\"jr-insta-wrap\">";
					}

					$caption_list .= '<div class="jr-insta-datacontainer">\n';
					$caption_list .= '<span class="jr-insta-time">{{timestamp}}</span>\n';
					$caption_list .= '<span class="jr-insta-username">by <a rel="nofollow noreferrer" href="https://www.instagram.com/{{username}}/" target="_blank">{{username}}</a></span>\n';
					$caption_list   .= '<span class="jr-insta-caption">{{caption}}</span>\n';
					$caption_list .= '</div>\n';
      ...
      jQuery(window).on('load', function(){";
					...
					$output .= " var feed = new Instafeed({
      accessToken: '".$access_token."',
			target: list_".$widget_id.",
			limit: ".$images_number.",
			...
			$output .= 'template: \'<li>' .$image_list_start.'<img title="{{caption}}" alt="{{caption}}" "' . $class . '" src="{{image}}" />'.$image_list_end . $caption_list . '</li>\'
    });
    feed.run();';
					if (isset($image_lazyclass)) {
						$output .= "}
