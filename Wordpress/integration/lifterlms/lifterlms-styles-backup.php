...
function do_lifterlms_customizer_css() {
	...
	if ( $primary ) {
		...
		$primary_dark  = lifterlms_adjust_hex( $primary, -0.12 );
		$primary_light = lifterlms_adjust_hex( $primary, 0.08 );
		$primary_text  = lifterlms_get_luminance( $primary ) > 0.179 ? '#000' : '#fff';
		...
		.llms-notice {
			border-color: <?php echo $primary; ?>;
			background: <?php echo lifterlms_hex_to_rgb( $primary, 0.3 ); ?>;
		}
	...
	<?php } if ( $action ) {
		...
		$action_dark  = lifterlms_adjust_hex( $action, -0.12 );
		$action_light = lifterlms_adjust_hex( $action, 0.08 );
		$action_text  = lifterlms_get_luminance( $action ) > 0.179 ? '#000' : '#fff';
...
add_action( 'after_customizer_css', 'do_lifterlms_customizer_css', 10 );
