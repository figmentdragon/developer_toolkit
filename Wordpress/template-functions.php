...
add_filter( 'post_class', 'creativity_post_classes', 10, 3 );
 ...
 * @return void
 */
function creativity_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'creativity_supports_js' );
 ...
 * @return array
 */
function creativity_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'creativity_comment_form_defaults' );
 ...
 * @return bool
 */
function creativity_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'creativity_can_show_post_thumbnail',
 ...
 * @return int
 */
function creativity_get_avatar_size() {
	return 60;
}
 ...
 * @since creativity 1.0
 */
function creativity_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'creativity' ),
 ...
 * @since creativity 1.0
 */
function creativity_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . creativity_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'creativity_continue_reading_link_excerpt' );
 ...
 * @since creativity 1.0
 */
function creativity_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . creativity_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'creativity_continue_reading_link' );

if ( ! function_exists( 'creativity_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function creativity_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'creativity' ) : $title;
	}
}
add_filter( 'the_title', 'creativity_post_title' );
 ...
 * @return string
 */
function creativity_get_icon_svg( $group, $icon, $size = 24 ) {
	return creativity_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function creativity_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? creativity_get_icon_svg( 'ui', 'arrow_right' ) : creativity_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? creativity_get_icon_svg( 'ui', 'arrow_left' ) : creativity_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'creativity_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function creativity_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'creativity_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'creativity_get_localized_font_family_elements',
		array(
			'front-end'      => array(
				'body',
				'input',
	...
	if ( ! function_exists( 'creativity_generate_css' ) ) {
	...
	return creativity_generate_css( // @phpstan-ignore-line.
 ...
 */
function creativity_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since creativity 1.0
 * @since creativity 1.4 Corrected parameter name for `$output`,
 ...
 */
function creativity_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'creativity' ) . '</p>'
	'<form action=' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . 'class="post-password-form"' 'method="post">'
	'<label class="post-password-form__label"' 'for=""' . esc_attr( $label ) . '>' . esc_html_x( 'Password', 'Post password form', 'creativity' ) . '</label>''<input class="post-password-form__input"' 'name="post_password"' 'id=""' . esc_attr( $label ) . 'type="password"' 'size="20"' /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'creativity' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'creativity' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'creativity_password_form', 10, 2 );
 ...
 */
function creativity_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'creativity_get_attachment_image_attributes', 10, 3 );
