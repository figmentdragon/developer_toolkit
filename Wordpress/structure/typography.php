...
if ( ! function_exists( 'themename_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'themename_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function themename_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', themename_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $themename_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $themename_settings[$key], ':' ) !== false ) {
					$themename_settings[$key] = current( explode( ':', $themename_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $themename_settings[$key] );

				// Grab the variants using the plain name
				$variants = themename_get_google_font_variants( $themename_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'themename_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'themename_fonts_subset','' );
		...
		$default_fonts = themename_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('themename-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('themename-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'themename_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'themename_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function themename_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'themename_fonts_customize_register' ) ) {
			// Bail if themename Premium is active
			return;
		}

		$defaults = themename_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'themename_Typography_Customize_Control' );
			$wp_customize->register_control_type( 'themename_Range_Slider_Control' );
		}
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'themename_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'themename_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'themename_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new themename_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'themename' ),
					'settings' => array(
						'family' => 'themename_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'themename_settings[body_font_weight]',
						'transform' => 'themename_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'themename_settings[body_font_size]',
			array(
				'default' => $defaults['body_font_size'],
				'type' => 'option',
				'sanitize_callback' => 'themename_sanitize_integer',
				'transport' => 'postMessage'
			)
		);

		$wp_customize->add_control(
			new themename_Range_Slider_Control(
				$wp_customize,
				'themename_settings[body_font_size]',
				array(
					'type' => 'themename-range-slider',
					'description' => __( 'Font size', 'themename' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'themename_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'themename_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'themename_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new themename_Range_Slider_Control(
				...
				array(
					'type' => 'themename-range-slider',
					'description' => __( 'Line height', 'themename' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'themename_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'themename_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new themename_Range_Slider_Control(
				...
				array(
					'type' => 'themename-range-slider',
					'description' => __( 'Paragraph margin', 'themename' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'themename_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'themename_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function themename_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'themename_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'themename_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'themename_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_themename_get_all_google_fonts_ajax', 'themename_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function themename_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'themename_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'themename_customize_nonce' ] ), 'themename_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'themename_customize_nonce', 'themename_customize_nonce' );
		...
		$fonts = themename_get_all_google_fonts();
...
if ( ! function_exists( 'themename_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: themename_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function themename_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, themename_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'themename_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'themename_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: themename_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function themename_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'themename_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function themename_get_font_family_css( $font, $settings, $default ) {
		$themename_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'themename_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $themename_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'themename_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . themename_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'themename_add_to_font_customizer_list' ) ) {
	add_filter( 'themename_typography_customize_list', 'themename_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function themename_add_to_font_customizer_list( $fonts ) {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'themename_get_default_fonts' ) ) {
			return;
		}
		...
		$all_fonts = themename_get_all_google_fonts();
		$select_fonts = themename_get_all_google_fonts( apply_filters( 'themename_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $themename_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $themename_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $themename_settings[ $setting ], themename_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $themename_settings[ $setting ] ) ) ] = array(
				'name' => $themename_settings[ $setting ],
		...
		if ( function_exists( 'themename_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'themename_secondary_nav_settings', array() ),
				themename_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], themename_typography_default_fonts() ) ) {
