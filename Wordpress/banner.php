...
if (creativity_option('creativity_lazyload_image_banner') == true) {
	function creativity_filter_gallery_img_atts( $atts, $attachment, $size ) {
			...
			$atts['class'] .= ' lazyload ' . creativity_option('creativity_lazyload_effect');
	    return $atts;
	}
	add_filter( 'wp_get_attachment_image_attributes', 'creativity_filter_gallery_img_atts', 10, 3 );
}
...
function creativity_default_image() {
    global $creativity_data;
    if ($creativity_data['creativity_default_image']) {
        return $creativity_data['creativity_default_image'];
    } else {
        return get_template_directory_uri() . '/img/default.jpg';
    }
}

function creativity_video_prov($vurl) {
    ...
    function creativity_video_id($prov, $vurl) {
    	if ($prov == 'youtube') {
...
function creativity_video_frame($prov, $vid) {
    global $creativity_frame_tag;
    global $lazyclass, $src;
    echo '<div class="video_fit_container">';
    if ($prov == 'youtube') {
        ?>
          <<?php echo esc_attr($creativity_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>?wmode=transparent&wmode=opaque" frameborder="0" allowfullscreen></<?php echo esc_attr($creativity_frame_tag); ?>>
        <?php
    } elseif ($prov == 'youtu') {
        ?>
        <<?php echo esc_attr($creativity_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//www.youtube.com/embed/<?php echo esc_attr($vid); ?>" frameborder="0" allowfullscreen></<?php echo esc_attr($creativity_frame_tag); ?>>
        <?php
    } elseif ($prov == 'vimeo') {
        ?>
        <<?php echo esc_attr($creativity_frame_tag); ?>  class="video_frame <?php echo esc_attr($lazyclass);?>" <?php echo esc_attr($src);?>="//player.vimeo.com/video/<?php echo esc_attr($vid); ?>?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></<?php echo esc_attr($creativity_frame_tag); ?>>
        <?php
    } else {
...
if (!function_exists('creativity_blog_post_banner')) {
function creativity_blog_post_banner($size = "", $class = "") {
    global $creativity_frame_tag;
    global $post;
    global $lazyclass, $src;
    ...
    if ($size == '') {
      if (is_single()) {
        $size = "post-thumbnail";
				if ((creativity_cross_option('creativity_sidebar_position', $post->ID) != 'none') && is_active_sidebar( 'sidebar-1' ) && creativity_option('creativity_banners_devices_size')) {
    ...
    if (get_post_format() == "image" ) {
        ...
        if (is_single()) {
            the_post_thumbnail($size, array("class"=>"img-responsive") );
						...
						if (creativity_option('creativity_banner_single_caption')) {
          ...
          if (creativity_cross_option('creativity_single_thumb_crop') == 'no') {
          ...
          the_post_thumbnail($size, array("class"=>"img-responsive") );
            ...
            echo '</a>';
						...
						if (creativity_option('creativity_banner_blog_caption')) {
							if (get_post(get_post_thumbnail_id())->post_excerpt) { // search for if the image has caption added on it
								echo '<figcaption class="wp-caption wp-caption-text">' . wp_kses_post(get_post(get_post_thumbnail_id())->post_excerpt) . '</figcaption>'; // displays the image caption
							}
						}
        ...
        if (get_post_meta($post->ID, '_format_video_embed', true) != '' ) {
            ...
            if (strpos($video_url, $creativity_frame_tag) != false) {
                echo '<div class="video_fit_container">';
                ...
                $prov = creativity_video_prov($video_url);
                $vid = creativity_video_id($prov, $video_url);
                creativity_video_frame($prov, $vid);
            }
            echo '</div>';
						wp_enqueue_script('creativity-fitvids-script');
        }else{
            // if video form empty
        }

    } elseif (get_post_format() == "gallery") {
			if (!is_single() && creativity_option('creativity_blog_gallery_crop') == 'no') {
				$size = 'full';
			}
      if (((class_exists( 'Jetpack' )) && (Jetpack::is_module_active( 'carousel' )))) {
        ...
        echo '</div>';
      } elseif (creativity_option('creativity_deactivate_theme_gallery')) {
        echo '<div class="blog_post_banner blog_post_'.get_post_format().''.$class.'">';
        $gallery_shortcode = get_post_meta($post->ID, '_format_gallery_shortcode', true);
        ...
        if (get_post_meta($post->ID, '_format_gallery_shortcode', true) != '' ) {
            ...
            echo '<div id="gallery_of_post_'.$post->ID.'" class="clearfix gallery galleryofpostid-'.$post->ID.' creativity_row gallery_row creativity_post_gallery creativity_post_gallery_attachements ">';
            ...
            echo '<div class="creativity_post_gallery_nav_container clearfix"></div>';
      	...
      	echo '</div>';
			...
			wp_enqueue_script('creativity-imagesloaded-script');
			wp_enqueue_script('creativity-gallery-script');

    } elseif (get_post_format() == "audio") {
        echo '<div class="blog_post_banner blog_post_'.get_post_format().''.$class.'">';
        $sound_url = get_post_meta($post->ID, '_format_audio_embed', true);
        if (strpos($sound_url, $creativity_frame_tag) != false) {
            echo '<div class="video_fit_container">';
            ...
            ?>
            <<?php echo esc_attr($creativity_frame_tag); ?> height="166" <?php if ($lazyclass != '') {?>class="lazyload"<?php } ?> style="overflow:hidden;border:none;width:100%" <?php echo esc_attr($src);?>="https://w.soundcloud.com/player/?url=<?php echo esc_url($sound_url); ?>"></<?php echo esc_attr($creativity_frame_tag); ?>>
            <?php
        }
        echo '</div>';
    } else {
