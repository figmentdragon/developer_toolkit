...
function editlink_meta_box() {

	add_meta_box(
		're_editlink',
		'Author Re-Edit Link',
		'editlink_meta_box_callback',
		'post',
		'side'
	);
}
...
function editlink_meta_box_callback( $post ) {

	// Add a nonce field so we can check for it later.
	wp_nonce_field( 'editlink_meta_box_data', 'editlink_meta_box_nonce' );
	...
	if ( !$ekey ) {
		make_edit_link( $post->ID, $post->post_title );
		$ekey = get_post_meta( $post->ID, 'wEditKey', 1 );
	}
	...
	echo '</label> ';
	echo '<input style="width:100%; type="text" id="writing_edit_link" name="writing_edit_link" value="' . get_bloginfo('url') . '/' . get_write_page() . '/?wid=' . $post->ID . '&tk=' . $ekey  . '"  onclick="this.select();" />';

	}


function make_edit_link( $post_id, $post_title='') {
...
function get_edit_link( $post_id ) {
	return ( get_bloginfo('url') . '/' . get_write_page() . '/?wid=' . $post_id . '&tk=' . get_post_meta( $post_id, 'wEditKey', 1) );

}

function mail_edit_link ( $wid, $mode = 'request' )  {
	...
	$edit_instructions = '<p>To be able to edit this work use this special access link <a href="' . get_bloginfo('url') . '/' . get_write_page() . '/?wid=' . $wid . '&tk=' . $wEditKey  . '">' . get_bloginfo('url') . '/' . get_write_page() . '?wid=' . $wid . '&tk=' . $wEditKey  . '</p>It should open your last edited version so you can make any modifications to it. Save this email as a way to always return to edit your writing or use the Request Edit Link button at the bottom of your published work.</p>';
